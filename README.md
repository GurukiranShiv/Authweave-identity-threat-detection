...
**Identity Telemetry Normalization and Threat Detection Platform**

AuthWeave is a SOC-focused identity security platform that accepts identity, SaaS, VPN, and audit telemetry from different log formats, normalizes the data into a unified identity schema, runs threat detections, scores risk, and generates analyst-ready investigation output.

The project is designed to avoid fixed, single-vendor parsing. Its adaptive log normalization engine inspects both field names and sample values to map custom logs into a common schema before running detections.
...
- SOC investigation brief generator
- MITRE ATT&CK mapping for generated alerts
- SIEM-ready exports for Splunk SPL, Microsoft KQL, and Elastic KQL
...
Save your screenshots inside the `screenshots/` folder using the exact names below. GitHub will render them automatically in this README.

### 1. Dashboard and File Upload

![Dashboard Upload](screenshots/01-dashboard-upload.png)

The main dashboard allows analysts to upload identity and security logs using a drag-and-drop interface. The sidebar explains the project flow from file upload to SIEM-ready exports.
...
The Field Mapping tab shows how AuthWeave converts vendor-specific or custom fields into normalized schema fields such as `user.email`, `src.ip`, `src.country`, `app.name`, and `event.outcome`.

### 4. SOC Investigation Brief

![SOC Investigation Brief](screenshots/04-soc-investigation-brief.png)

The SOC Investigation Brief tab converts structured detection output into an analyst-friendly summary with evidence, risk explanation, and response steps.
...
| Concept | Microsoft Entra ID | Okta | Google Workspace | AuthWeave Normalized Field |
|---|---|---|---|---|
...
AuthWeave solves this by normalizing different log sources before running detections.
...
CSV / JSON / NDJSON / TXT / LOG / XLSX
  ...
  -> Field and value analyzer
  -> Adaptive field mapper
  -> Normalized identity schema
  -> Threat detection engine
  -> Risk scoring and severity classification
  -> SOC investigation summarizer
  -> Dashboard and SIEM-ready exports
```

## Adaptive Log Normalization

AuthWeave is not limited to one predefined file format.
...
- TXT and LOG files with key-value style records
...
| `event.type` | Login, OAuth consent, role change, SaaS action, mailbox rule, or related activity |
| `event.outcome` | Success, failure, denied, blocked, or approved |
...
| `src.device` | Device, operating system, browser, or endpoint info |
...
AuthWeave currently includes the following detections:
...
When you run the CLI pipeline, AuthWeave writes analyst-ready files into the selected output folder.

| Output File | Purpose |
|---|---|
| `normalized_events.json` | Clean normalized events after field mapping |
| `mapping_profile.yaml` | Source-to-normalized field mapping with confidence and reasoning |
...
| `soc_investigation_brief.md` | SOC investigation summary |
...
```text
authweave-identity-threat-detection/
  app.py
  api.py
  run_pipeline.py
  requirements.txt
  ...
  screenshots/
    ...
    04-soc-investigation-brief.png
    05-siem-exports.png
  sample_data/
    entra_signins.csv
    okta_logs.json
...
```powershell
cd authweave-identity-threat-detection
```
...
AuthWeave works best when the file contains meaningful identity or security values such as:
...
- MFA status
- OAuth scopes
- role or admin fields
- mailbox forwarding fields
- SaaS admin actions

If a file is unrelated, such as a finance or sales spreadsheet, AuthWeave may parse the file but will not generate meaningful identity threat detections.
...
## SOC Investigation Brief

The investigation brief is generated after the detection engine. It does not replace detection logic.

Correct flow:

```text
Normalized logs -> Detection engine -> Risk score -> SOC investigation brief
```
...
- Save reusable mapping profiles per vendor or source
...
- Add optional local/offline LLM summarization
- Add unit tests and GitHub Actions CI pipeline

## GitHub Repository Description

```text
AuthWeave normalizes identity, SaaS, cloud, and VPN telemetry into a unified schema, then detects risky sign-ins, impossible travel, MFA abuse, OAuth threats, privilege escalation, and suspicious account activity with SOC-ready summaries and SIEM exports.
```
